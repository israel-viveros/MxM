(function() {
    ...
    var pathMethods = Sonic.pathMethods = {
        ...
        arc: function(t, cx, cy, radius, start, end) {
            ...
            var ret = [
                (Math.cos(point) * radius) + cx, (Math.sin(point) * radius) + cy
            ];
...
$(document).ready(function() {
    ...
    square.play();
    if (document.getElementById("MxMLoaderTIM")) {
        document.getElementById("MxMLoaderTIM").appendChild(square.canvas);
    }

    setInterval(function() {
        if ($("#MxMLoaderTIM").length) {
            $("#MxMLoaderTIM").fadeOut('slow', function() {
                $(this).remove();
            });
        }
    }, 600);

    if ($(".see-all.textcolor-title4").length) {
        $(".see-all.textcolor-title4").unbind().click(function(event) {
            var colapsablesimbo = $("div.alineacion_partido").height();
...
$(window).load(function() {
    if ($("#MxMLoaderTIM").length) {
        ...
        $("#MxMLoaderTIM").click(function(event) {
            event.preventDefault();
