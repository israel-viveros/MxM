 ...
 */
(function(){$.fn.wdgAltasbajas=function(a){var c=$.extend({idTournament:0,nameTournament:"",abbody:false,title:""},a);
...
h<j.dataEstadistica.length;h++){if(h<18){g+='<td class="textcolor-title3" id="lg'+j.dataEstadistica[h].idTeam+'">';g+='<a href="'+document.URL.replace(/[\\\/][^\\\/]*$/,"")+"/"+j.dataEstadistica[h].webNameTeam+'_d.html" target="_parent" class="underline_text">';
g+='<img src="'+j.dataEstadistica[h].urlLogoClub+'" width="24" height="24" alt="'+j.dataEstadistica[h].aliasTeam+'">'+j.dataEstadistica[h].aliasTeam;g+="</a>";
g+="</td>";e+='<td class="al'+j.dataEstadistica[h].idTeam+'">-</td>';f+='<td class="ba'+j.dataEstadistica[h].idTeam+'">-</td>';}}$("#firstlogo").html(g);
$("#altTIM").html(e);$("#bajTIM").html(f);try{if(c.abbody===true){d.secondDraw(j);}else{d.infodraft();}}catch(k){console.log(k);}}).fail(function(){console.log("error al cargar el feed: "+d.feedEquipos);
});},infodraft:function(){var f="",e=0;$.ajax({url:d.feedDraft,type:"GET",dataType:"jsonp",jsonpCallback:"teamDraft",cache:false}).done(function(j){var m=j.draftEndDate.split("/"),g=m[2]+"/"+m[1]+"/"+m[0],l=g+" "+j.draftEnd;
...
g+='<div class="teams_stadistics_table">';for(var e=0;e<f.dataEstadistica.length;e++){g+='<div class="teams_stadistics">';g+='<a class="link_table" target="_self" href="'+document.URL.replace(/[\\\/][^\\\/]*$/,"")+"/"+f.dataEstadistica[e].webNameTeam+'_d.html">';
