; var IdEventoMxmV = (typeof IdEventoMxm ==="undefined" || IdEventoMxm === "") ? 0 : IdEventoMxm;
var IdEventoTVMxMV = (typeof IdEventoTVMxM === "undefined" || IdEventoTVMxM === "" ) ? 0 : IdEventoTVMxM;
...
$(document).ready(function(){
	...
	function ocultar()
	{
		...
		$('.wdg_mxm_live_02 iframe').attr('src','');
		$('.tvsa-camera').show(); 
	};
	//Click camera
    $('.wdg_mxm_live_02 .tvsa-camera').live('click',function(event){
		$('.tvsa-camera').removeClass("active");  
		activo_cam = $(this).attr('class');
    ...
    $('.wdg_mxm_live_02 .tvsa-videocamera').live('click',function(event){ 
		...
		if($(this).parent().siblings('.vid_player_01').css('display')!='none'){
			var wi=0, hi=0;
			wi=$(this).parent().siblings('.vid_player_01').find('iframe').outerWidth();
			//hi=$(this).parent().siblings('.vid_player_01').find('iframe').outerHeight();
			
			//$('.vid_player_01_image').css('height',parseInt($('.vid_player_01_image').width()*9/16));
			
			//console.log("tama̱o: "+$('.vid_player_01_image').width()*9/16);
			hi=$('.vid_player_01_image').width()*9/16;
			if (parseInt(hi)==0)
			{
				hi=185;
			}
			//$('#vid'+id).attr('data', 'http://www.tvolucion.com/embed/embed_ampp.php?id=' + idVid+ '&w='+w+'&h='+h+'');
			var currentVideo = $(this).parent().siblings('.vid_player_01').find('iframe').attr('data-id');
			$(this).parent().siblings('.vid_player_01').find('iframe').attr('src','http://tv.televisadeportes.esmas.com/embed/embed_ampp.php?id='+currentVideo+ '&w='+wi+'&h='+hi+'');
		}else{
			$(this).parent().siblings('.vid_player_01').find('iframe').attr('src','');
		}
        $(this).toggleClass("textcolor-title1");
