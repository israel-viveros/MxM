...
MaqueWdgAltas+='<span class="tvsa-caret-up"></span>';MaqueWdgAltas+="</a>";MaqueWdgAltas+='<a class="next bginactive" href="#">';MaqueWdgAltas+='<span class="tvsa-caret-down"></span>';
MaqueWdgAltas+="</a>";MaqueWdgAltas+='<a class="full-timetable" href="#"> ';MaqueWdgAltas+="<span>Ver todos</span> ";MaqueWdgAltas+="</a>";MaqueWdgAltas+="</div>";
}MaqueWdgAltas+='<div class="degraded"></div>';h.css("display","none").html(MaqueWdgAltas);clearInterval(c.timerCalendar);if(e.tema!=="mundial"){c.timerCalendar=setInterval((function(){c.actualizaContenido();
...
var m=k.getTime()/1000;$.when(c.primeraJornada()).done(function(){setTimeout(function(){c.segundaJornada();},800);});if(c.numeroTorneoAct!==0&&c.numeroIdEquipo===0){urFinal=c.jornadaCalendarRoute+c.numeroTorneoAct+"/jornadas/jornadalistadojsonp.js";
...
h.find(".division h2").text(S[L].tournament.name);if(S[L].tournament.icono.length>0){h.find("#ALtournamentLogo").attr("src",S[L].tournament.icono);}else{h.find("#ALtournamentLogo").hide();
}(typeof(S[L].sef)!=="undefined"&&typeof(S[L].sef.tournamentid)!=="undefined")?h.find(".controls .full-timetable").attr("href","http://stats.televisadeportes.esmas.com/futbol/torneo/"+String(S[L].tournament.name).replace(/ /g,"-").toLowerCase()+"/calendario/"+S[L].sef.tournamentid+"/"):"";
