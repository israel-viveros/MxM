...
}).fail(function(g){console.log("Error al cargar: "+e.urlFinalHeader);console.log(g);});},drawHeader:function(g){var h='<img class="linkbanderalocal" src="'+g.equipoLocal.lineupImage+'" alt="'+g.equipoLocal.nombre+'" width="48" height="36">';
var f='<img class="linkbanderavisit" src="'+g.equipoVisitante.lineupImage+'" width="48" height="36" alt="'+g.equipoVisitante.nombre+'">';var j='<span class="linkbanderalocal">'+g.equipoLocal.nombre+"</span>";
var k='<span class="linkbanderavisit">'+g.equipoVisitante.nombre+"</span>";var m="",l=(parseInt(g.minuto)!==0)?g.minuto+"'":"";m+='<div class="wrapper"><div class="match_title">';
m+='<span class="hidden" id="datosTIMHeader"><span id="linklocalTIM">'+g.equipoLocal.url+'</span> <span id="linkvisitTIM">'+g.equipoVisitante.url+'</span> <span id="timeUpdateMxM">0</span> <span id="localAbrevTIM" class="hidden">'+g.equipoLocal.abrev+'</span> <span id="visitAbrevTIM" class="hidden">'+g.equipoVisitante.abrev+'</span> <span id="localImgTIM" class="hidden">'+g.equipoLocal.smallImage+'</span> <span id="visitImgTIM" class="hidden">'+g.equipoVisitante.smallImage+'</span> <span id="localGolesTIM" class="hidden">'+g.equipoVisitante.goles+'</span> <span id="visitGolesTIM" class="hidden">'+g.equipoVisitante.goles+"</span>  </span>";
m+='<div class="cup_name">';m+=(g.transmisionVivo!=="")?'<div class="live-container textcolor-title3 background-color2 hidden" id="TIMVivoHeader" onclick="javascript:window.open(\''+g.transmisionVivo+'\');" style="cursor:pointer"><div class="icon-video"><i class="tvsa-videocamera"></i></div><div class="see-now">VER AHORA</div><div class="online">EN VIVO</div></div>':"";
m+='<div class="titulo textcolor-title3">'+g.torneo.nombre+"</div>";m+='<div class="subtitulo textcolor-title2">'+g.jornada.nombre+"</div>";m+="</div>";
m+="</div>";m+='<div class="live_time textcolor-title3 background-color1"><span>'+g.tiempo.replace(/'/g,"'")+" "+l+"</span></div>";m+='<div class="leftside">';
m+=(c.tema==="mundial")?'<div class="padre">':"";m+='<div class="team1">';m+='<div class="escudo">'+h+"</div>";m+='<div class="equipo">'+j+"</div>";m+='<div class="score">'+g.equipoLocal.goles+"</div>";
m+=(g.equipoLocal.penales===""||parseInt(g.equipoLocal.penales)===0)?'<div class="penales" style="visibility:hidden"><span class="penal"></span> PENALES</div>':'<div class="penales"><span class="penal">'+g.equipoLocal.penales+"</span> PENALES</div>";
m+="</div>";m+='<div class="dotted_separator"></div>';m+='<div class="team2">';m+=(g.equipoVisitante.penales===""||parseInt(g.equipoVisitante.penales)===0)?'<div class="penales" style="visibility:hidden"><span class="penal"></span> PENALES</div>':'<div class="penales"><span class="penal">'+g.equipoVisitante.penales+"</span> PENALES</div>";
m+='<div class="score">'+g.equipoVisitante.goles+"</div>";m+='<div class="equipo">'+k+"</div>";m+='<div class="escudo">'+f+"</div>";m+="</div>";m+='<div class="team2_m">';
m+=(g.equipoVisitante.lineupImage!=="")?'<div class="escudo">'+f+"</div>":"";m+='<div class="equipo">'+k+"</div>";m+='<div class="score">'+g.equipoVisitante.goles+"</div>";
m+=(g.equipoVisitante.penales===""||parseInt(g.equipoVisitante.penales)===0)?'<div class="penales" style="visibility:hidden"><span class="penal"></span> PENALES</div>':'<div class="penales"><span class="penal">'+g.equipoVisitante.penales+"</span> PENALES</div>";
m+="</div>";m+=(c.tema!=="mundial")?'<div class="solid_separator"></div>':"";m+='<div class="match_info">';m+=(typeof g.partidoIda!=="undefined")?'<div class="ida">Partido ida: <span class="blanco">'+g.partidoIda.local+" "+g.partidoIda.golesLocal+" - "+g.partidoIda.golesVisitante+" "+g.partidoIda.visitante+"</span></div>":"";
if(parseInt(g.equipoLocal.golesGlobal)!==0||parseInt(g.equipoVisitante.golesGlobal)!==0){m+='<div class="scoreglobal">Global: <span class="blanco">'+g.equipoLocal.nombre+' <span id="globalLoc">'+g.equipoLocal.golesGlobal+'</span> - <span id="globalVi">'+g.equipoVisitante.golesGlobal+"</span> "+g.equipoVisitante.nombre+"</span></div>";
}m+="</div>";m+="</div>";m+=(c.tema==="mundial")?'<div class="solid_separator"></div></div>':"";m+='<div class="date_venue">';m+='<div class="when">'+g.fechaPartidoLetra.replace(/-/g," ")+" <span>"+g.horaPartido+"</span></div>";
m+=(typeof g.estadio!=="undefined")?'<div class="where">Estadio '+g.estadio.nombre+"<br>"+g.estadio.ciudad+", "+g.estadio.pais+"</div>":"";m+=(typeof g.datocurioso!=="undefined")?'<div class="info">'+g.datocurioso+"</div>":"";
m+="</div>";m+="</div>";a.css("display","none").html(m).slideDown("slow",function(){$(this).css({display:"block"});});if(g.tiempo.toLowerCase()!=="final"){setTimeout(function(){e.timeUpdate(g.fechaPartido,g.horaPartido);
...
var n=$("#linklocalTIM").text();var i=$("#linkvisitTIM").text();$(document).on("click",".linkbanderalocal",function(o){o.preventDefault();(n!=="")?window.open(n,"_blank"):"";
});$(document).on("click",".linkbanderavisit",function(o){o.preventDefault();(i!=="")?window.open(i,"_blank"):"";});$(document).on("hover",".linkbanderalocal",function(){if(n!==""){$(this).css("cursor","pointer");
}});$(document).on("hover",".linkbanderavisit",function(){if(i!==""){$(this).css("cursor","pointer");}});},updateGoles:function(k){var h="",h="",i="",m="",n="",r="",u="",j="",f="",l="",g="",t="",s="",q="",p="",o=(parseInt(k.minuto)!==0)?k.minuto+"'":"";
