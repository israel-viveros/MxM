...
f+="</div>";f+='<div class="dotted_separator"></div>';f+='<div class="team2">';f+=(g.equipoVisitante.penales===""||parseInt(g.equipoVisitante.penales)===0)?'<div class="penales" style="visibility:hidden"><span class="penal"></span> PENALES</div>':'<div class="penales"><span class="penal">'+g.equipoVisitante.penales+"</span> PENALES2</div>";
...
$.ajax({url:"http://mxm.televisadeportes.esmas.com/deportes/home/timetvjsonp.js",async:false,cache:false,dataType:"jsonp",jsonpCallback:"timetv",success:function(r){a.find(".score").css("visibility","hidden");
var t="";var o=0;var n=0;horas=r.timetv;t=r.fechatv.replace(/_/gi,"-").split("-");o=parseInt(t[1])+1;if(String(o).length==1){o="0"+o;}n=parseInt(t[2])+1900;
fechas=n+"/"+o+"/"+t[0];fechas=fechas+" "+horas+":00";var y=new Date(i);var x=new Date(fechas);var s=Date.UTC(y.getFullYear(),y.getMonth()+1,y.getDate());
var q=Date.UTC(x.getFullYear(),x.getMonth()+1,x.getDate());if(parseFloat(s)<parseFloat(q)){console.log("Fecha Menor(YA PASO EL PARTIDO)");a.find(".score").css("visibility","visible");
}else{if(parseFloat(s)==parseFloat(q)){console.log("Dia del Evento");var v=y.getTime(),l=x.getTime(),k=9000000,u=900000,w=v+k,z=v-u,p=v-l;console.log("Hora Servidor: "+new Date(l));
console.log("Hora Partido: "+new Date(v));console.log("Hora Previo partido: "+new Date(z));console.log("Hora Fin partido: "+new Date(w));if(v<=l&&l<=w){console.log("El patido esta en vivo");
if(f.length){f.removeClass("hidden");}h=30000;a.find(".score").css("visibility","visible");}else{console.log(p);if(parseInt(p)<u&&parseInt(p)>0){console.log("Estamos a menos de 15 min del partido");
h=300000;}}}else{if(parseFloat(s)>parseFloat(q)){console.log("Fecha Mayor (AUN NO PASA EL PARTIDO)");}else{console.log("Error no actualizo");}}}console.log("Tiempo de actualizacion: "+h);
clearInterval(d);if(h!==0){d=setInterval(function(){e.tmpescuchaListener=parseInt($("#timeUpdateMxM").text());e.loadInfo("update");},h);}$("#timeUpdateMxM").text(h);
}});},drawMenu:function(h){var g="";g+='<div class="navarrowleft">';g+='<a class="wdg_matchesresult_navleft" href="#left"> ';g+='<span class="navlefticon"><i class="tvsa-double-caret-left inactive"></i></span></a></div>';
g+='<div class="container">';g+='<div class="nav_smnu_sports_01_bar"><ul>';g+=(typeof h.previo!=="undefined"&&h.previo!=="")?'<li class="previoMenuTim"> <a href="'+h.previo+'" target="_parent">Previo</a></li>':"";
g+=(typeof h.alineacion!=="undefined"&&h.alineacion!=="")?'<li class="nav_smnu_sports_01_block alineacionMenuTim"><a href="'+h.alineacion+'" target="_parent" title="Alineaci\u00F3n">Alineaci\u00F3n</a></li>':"";
g+=(typeof h.rating!=="undefined"&&h.rating!==""&&c.tema!=="mundial")?'<li class="hide1 ratingMenuTim"><a href="'+h.rating+'" title="Rating">Rating</a></li>':"";
g+=(typeof h.mxm!=="undefined"&&h.mxm!=="")?'<li class="nav_smnu_sports_01_block nav_smnu_sports_01_block2 mxmMenuTim"><a href="'+h.mxm+'" target="_parent" title="MxM">MxM</a></li>':"";
g+=(typeof h.pizarra!=="undefined"&&h.pizarra!==""&&c.tema!=="mundial")?'<li class="hide2 pizarraMenuTim"><a href="'+h.pizarra+'" title="Pizarra">Pizarra</a></li>':"";
g+=(typeof h.cronica!=="undefined"&&h.cronica!=="")?'<li class="nav_smnu_sports_01_block nav_smnu_sports_01_block2 cronicaMenuTim"><a href="'+h.cronica+'" target="_parent" title="Cr\u00F3nica">Cr\u00F3nica</a></li>':"";
g+=(typeof h.video!=="undefined"&&h.video!=="")?'<li class="last nav_smnu_sports_01_block videoMenuTim"><a href="'+h.video+'" title="Video" target="_parent">Video</a></li>':"";
g+=(c.tema==="mundial")?'<li class="last nav_smnu_sports_01_block interaMenuTim"><a href="interacciontd.html" title="interacci\u00F3n TD" target="_parent">Interacci\u00F3n TD</a></li>':"";
...
e.IdPestanasMenu.html(g).show();var f=document.URL;(/previo.html/.test(f))?$(".previoMenuTim").addClass("current"):"";(/previo_m.html/.test(f))?$(".previoMenuTim").addClass("current"):"";
(/alineacion.html/.test(f))?$(".alineacionMenuTim").addClass("current"):"";(/alineacion_m.html/.test(f))?$(".alineacionMenuTim").addClass("current"):"";
(/rating.html/.test(f))?$(".ratingMenuTim").addClass("current"):"";(/mxm.html/.test(f))?$(".mxmMenuTim").addClass("current"):"";(/mxm_m.html/.test(f))?$(".mxmMenuTim").addClass("current"):"";
(/pizarra.html/.test(f))?$(".pizarraMenuTim").addClass("current"):"";(/cronica.html/.test(f))?$(".cronicaMenuTim").addClass("current"):"";(/cronica_m.html/.test(f))?$(".cronicaMenuTim").addClass("current"):"";
(/video.html/.test(f))?$(".videoMenuTim").addClass("current"):"";(/interacciontd.html/.test(f))?$(".interaMenuTim").addClass("current"):"";}};if(c.idtorneo!=0&&c.idteam!==0){setTimeout(function(){e.loadInfo();
},300);}else{(console.log("---->Faltan los Id's de team y/o torneo"));}};})(jQuery);