...
var a=this;var d={urlFinalHeader:"http://static-televisadeportes.esmas.com/sportsdata/futbol/data/"+c.idtorneo+"/"+c.idteam+"/match_header.js",IdPestanasMenu:$("#TIMnav_smnu_sports"),loadInfo:function(e){$.ajax({url:d.urlFinalHeader,type:"GET",dataType:"jsonp",jsonpCallback:"mxmheader",cache:false,async:false}).done(function(f){(e==="update")?d.updateGoles(f):d.drawHeader(f);
}).fail(function(f){console.log("Error al cargar: "+d.urlFinalHeader);console.log(f);a.hide();});},drawHeader:function(f){var e="",g=(parseInt(f.minuto)!=0)?f.minuto+"'":"";
e+='<div class="wrapper"><div class="match_title">';e+='<span class="hidden" id="datosTIMHeader"> <span id="timeUpdateMxM">0</span> <span id="localAbrevTIM" class="hidden">'+f.equipoLocal.abrev+'</span> <span id="visitAbrevTIM" class="hidden">'+f.equipoVisitante.abrev+'</span> <span id="localImgTIM" class="hidden">'+f.equipoLocal.smallImage+'</span> <span id="visitImgTIM" class="hidden">'+f.equipoVisitante.smallImage+'</span> <span id="localGolesTIM" class="hidden">'+f.equipoVisitante.goles+'</span> <span id="visitGolesTIM" class="hidden">'+f.equipoVisitante.goles+"</span>  </span>";
...
e+="</div>";e+='<div class="live_time textcolor-title3 background-color1"><span>'+f.tiempo.replace(/'/g,"'")+" "+g+"</span></div>";e+='<div class="leftside">';
...
},300);setInterval(function(){d.timeUpdate(f.fechaPartido,f.horaPartido);},60000);}(typeof f.paginas!=="undefined"&&d.IdPestanasMenu.length)?d.drawMenu(f.paginas):d.IdPestanasMenu.hide();
},updateGoles:function(j){var g="",g="",h="",l="",m="",q="",t="",i="",e="",k="",f="",s="",r="",p="",o="",n=(parseInt(j.minuto)!=0)?j.minuto+"'":"";var g=String(j.equipoLocal.goles),h=String(j.equipoVisitante.goles),l=String(j.equipoLocal.penales),m=String(j.equipoVisitante.penales),k=String(j.tiempo)+" "+String(n),o=String(j.equipoLocal.golesGlobal),r=String(j.equipoVisitante.golesGlobal);
q=$(".team1 .score").text(),t=$(".team2 .score").text(),i=$(".team1 .penal").text(),e=$(".team2 .penal").text(),f=$(".live_time").text(),p=$("#globalLoc").text(),s=$("#globalVi").text();
(g!==String(q))?d.nuevoValor($(".team1 .score"),g):"";(h!==String(t))?d.nuevoValor($(".team2 .score, .team2_m .score"),h):"";(i!==String(l)&&parseInt(l)>0)?d.nuevoValor($(".team1 .penal"),l):"";
(e!==String(m)&&parseInt(m)>0)?d.nuevoValor($(".team2 .penal,.team2_m .penal"),m):"";(k!==String(f))?d.nuevoValor($(".live_time span"),k):"";(o!==String(p))?d.nuevoValor($("#globalLoc"),o):"";
(r!==String(s))?d.nuevoValor($("#globalVi"),r):"";},nuevoValor:function(e,f){e.parent(".penales").css("visibility","visible");e.fadeOut("slow",function(){$(this).empty();
...
$.ajax({url:"http://mxm.televisadeportes.esmas.com/deportes/home/timetvjsonp.js",async:false,cache:false,dataType:"jsonp",jsonpCallback:"timetv",success:function(q){a.find(".score").css("visibility","hidden");
var s="";var n=0;var l=0;horas=q.timetv;s=q.fechatv.replace(/_/gi,"-").split("-");n=parseInt(s[1])+1;if(String(n).length==1){n="0"+n;}l=parseInt(s[2])+1900;
fechas=l+"/"+n+"/"+s[0];fechas=fechas+" "+horas+":00";var x=new Date(h);var w=new Date(fechas);var r=Date.UTC(x.getFullYear(),x.getMonth()+1,x.getDate());
var p=Date.UTC(w.getFullYear(),w.getMonth()+1,w.getDate());if(parseFloat(r)<parseFloat(p)){console.log("Fecha Menor(YA PASO EL PARTIDO)");a.find(".score").css("visibility","visible");
}else{if(parseFloat(r)==parseFloat(p)){console.log("Dia del Evento");var u=x.getTime(),k=w.getTime(),j=9000000,t=900000,v=u+j,y=u-t,o=u-k;console.log("Hora Servidor: "+new Date(k));
console.log("Hora Partido: "+new Date(u));console.log("Hora Previo partido: "+new Date(y));console.log("Hora Fin partido: "+new Date(v));if(u<=k&&k<=v){console.log("El patido esta en vivo");
if(e.length){e.removeClass("hidden");}g=30000;a.find(".score").css("visibility","visible");}else{console.log(o);if(parseInt(o)<t&&parseInt(o)>0){console.log("Estamos a menos de 15 min del partido");
g=300000;}}}else{if(parseFloat(r)>parseFloat(p)){console.log("Fecha Mayor (AUN NO PASA EL PARTIDO)");}else{console.log("Error no actualizo");}}}console.log("Tiempo de actualizacion: "+g);
if(g!==0){setInterval(function(){d.loadInfo("update");},g);}$("#timeUpdateMxM").text(g);}});},drawMenu:function(g){var f="";f+='<div class="navarrowleft">';
f+='<a class="wdg_matchesresult_navleft" href="#left"> ';f+='<span class="navlefticon"><i class="tvsa-double-caret-left inactive"></i></span></a></div>';
f+='<div class="container">';f+='<div class="nav_smnu_sports_01_bar"><ul>';f+=(typeof g.previo!=="undefined"&&g.previo!=="")?'<li class="previoMenuTim"> <a href="'+g.previo+'" target="_parent">Previo</a></li>':"";
f+=(typeof g.alineacion!=="undefined"&&g.alineacion!=="")?'<li class="nav_smnu_sports_01_block alineacionMenuTim"><a href="'+g.alineacion+'" target="_parent" title="Alineaci\u00F3n">Alineaci\u00F3n</a></li>':"";
...
(/video.html/.test(e))?$(".videoMenuTim").addClass("current"):"";(/interacciontd.html/.test(e))?$(".interaMenuTim").addClass("current"):"";}};if(c.idtorneo!=0&&c.idteam!==0){setTimeout(function(){d.loadInfo();
